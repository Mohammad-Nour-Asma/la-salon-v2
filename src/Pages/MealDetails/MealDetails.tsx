    ...
    return (
        ...
        >
            ...
            <Box
                ...
                <Typography color={"#333"} fontWeight={'600'} fontSize={'1.3rem'}>{language === 'ar' ? mealInfo?.meal?.ar_name : mealInfo?.meal?.name}</Typography>
                <Typography color={"#333"} m={"0 auto"} sx={{
                    width: { xs: '98%', sm: '80%', md: '70%', lg: '60%' }
                }}>{language === 'ar' ? mealInfo.meal?.ar_description : mealInfo.meal?.description}</Typography>
                <Box sx={{ textAlign: 'center', margin: '1rem auto', width: '60%' }}>
                    <Heading name={language === 'ar' ? 'ملاحظة الى المطبخ' : "note to kitchen"} />
                    <TextField
                        fullWidth
                        rows={2}
                        multiline
                ...
                <Stack sx={{
                    ...
                    <Link to={'/cart'} onClick={addToCartHandler} style={{
                        ...
                        borderRadius: '10px',
                        fontWeight: "bold"
                    }}>{language === 'ar' ? "اضافة الى السلة" : "Add To Cart"}</Link>
                </Stack>
            </Box>
        </motion.div>
    );
